...
const char DEVICE_KEY[]         = "LY@Vt0oYv3zkp04EPrRA44?d8";    //Enter Secret device password (Secret Key)
...
void onSwitch4Change(); //These functions are triggered when the cloud-controlled switches change state.

CloudSwitch switch1;
CloudSwitch switch2;
CloudSwitch switch3;
CloudSwitch switch4; //These variables store the state of cloud-controlled switches.
...
void initProperties(){
  ...
  ArduinoCloud.addProperty(switch3, READWRITE, ON_CHANGE, onSwitch3Change); //Registers the device with the cloud using DEVICE_ID and DEVICE_KEY
  ArduinoCloud.addProperty(switch4, READWRITE, ON_CHANGE, onSwitch4Change); //Links each cloud switch to its corresponding relay control function.

}

WiFiConnectionHandler ArduinoIoTPreferredConnection(SSID, PASS);  //This connects the ESP8266 to WiFi.

const int debounceDelay = 200; // Debounce delay in milliseconds
unsigned long lastDebounceTime1 = 0;
unsigned long lastDebounceTime2 = 0;
unsigned long lastDebounceTime3 = 0;
unsigned long lastDebounceTime4 = 0;  //Helps avoid multiple trigger signals when pressing a switch.
...
bool previousSwitchState4 = HIGH;  //Tracks the previous state of each switch to detect button press events.
...
void setup() {
  ...
  digitalWrite(wifiLed, HIGH);  //Turn OFF WiFi LED  // Initializes serial communication, WiFi, cloud connection, and pin modes.
}
...
}  //Continuously checks for cloud updates and manual switch inputs.
   //Controls WiFi LED to indicate connectivity.

void onSwitch1Change() {     //These functions are called when a cloud-controlled switch changes.
                             //Turns the relay ON/OFF based on the cloud command.
  if (switch1 == 1)
  {
